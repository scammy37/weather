...
export async function fetchStationDaily(stationId, start, end) {
  ...
  return { stationId, name, byDate, coverage: withTemp / expected, days: byDate.size };
}
...
export function mergeStationDaily(daily, station) {
  if (!daily || !daily.time || !station) return { replaced: 0, missing: 0, fields: [] };
  const keys = Object.values(STATION_FIELDS).filter(k => Array.isArray(daily[k]));
  ...
  daily.time.forEach((t, i) => {
    ...
    for (const k of keys) daily[k][i] = rec[k];
    replaced++;
  });
  return { replaced, missing, fields: keys };
}
